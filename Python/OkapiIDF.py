    ...
    while termcnt < totalterms:
        ...
        while i < maxlength:
            ...
            if finalvalues.has_key(doc_id):
                ...
                finalvalue.append([termcnt, (tf/(tf + 0.9 + (1.3 * doc_len / avgdoclen))) * (math.log10(numberofdocs/df))])
            else:
                finalvalues[doc_id] = [[termcnt, (tf/(tf + 0.9 + (1.3 * doc_len / avgdoclen))) * (math.log10(numberofdocs/df))]]
            i = i + 3
        count = count + df * 3 + 2
        termcnt = termcnt + 1    
